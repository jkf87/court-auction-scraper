...
### A. Claude Code (Chrome DevTools MCP)

개별 MCP 도구를 직접 호출한다:
```
mcp__chrome-devtools__new_page(url=URL)           # 페이지 열기
...
### B. OpenClaw / Pi / Antigravity (통합 browser 도구)

**단일 `browser` 도구**에 `action` 파라미터로 모든 동작을 수행한다:

```json
// 페이지 열기
{"action": "open", "targetUrl": "https://www.courtauction.go.kr/pgj/index.on"}
...
- 요소 참조: `ref="12"` (스냅샷의 숫자 ref) 또는 `ref="e12"` (role 스냅샷)
- **`evaluateEnabled=true` 필수** (기본값이 false일 수 있음)
- CLI: `openclaw browser open URL`, `openclaw browser snapshot` 등도 가능

### ⚠️ OpenClaw/Antigravity 접속 실패 시 체크리스트

법원경매 사이트 접속이 안 되는 경우:

1. **SSRF 정책 확인** — 기본 정책이 외부 사이트 접근을 차단할 수 있음:
   ```json
   {
     "browser": {
       "ssrfPolicy": {
         "hostnameAllowlist": ["*.courtauction.go.kr"]
       }
     }
   }
   ```

2. **프로필 확인** — `profile: "openclaw"` (관리 브라우저) vs `profile: "user"` (로컬 Chrome):
   ```json
   {"action": "open", "targetUrl": "URL", "profile": "user"}
   ```
   로컬 Chrome을 쓰려면 `--remote-debugging-port=9222`로 Chrome이 실행 중이어야 함.

3. **evaluateEnabled 확인**:
   ```json
   {"browser": {"evaluateEnabled": true}}
   ```

4. **target 확인** — sandbox/node/host 중 어디서 브라우저가 실행되는지:
   ```json
   {"action": "open", "targetUrl": "URL", "target": "host"}
   ```
...
### OpenClaw / Pi / Antigravity (통합 browser 도구)

세 플랫폼 모두 동일한 **`browser` 도구 + `action` 파라미터** 인터페이스를 사용한다:

**접속 실패 시 필수 체크:**

1. **SSRF 정책** — gateway/settings에서 courtauction.go.kr 허용:
   ```json
   {"browser": {"ssrfPolicy": {"hostnameAllowlist": ["*.courtauction.go.kr"]}}}
   ```

2. **프로필** — 로컬 Chrome 사용 시 `profile: "user"` + `--remote-debugging-port=9222`:
   ```json
   {"action": "open", "targetUrl": "URL", "profile": "user"}
   ```

3. **evaluateEnabled** — JS 실행 기능 활성화:
   ```json
   {"browser": {"evaluateEnabled": true}}
   ```

4. **target** — sandbox에서 외부 접근이 막힐 수 있음, host로 변경:
   ```json
   {"action": "open", "targetUrl": "URL", "target": "host"}
   ```

5. **브라우저 상태 확인**: `{"action": "status"}`

**Pi 추가 사항:**
- Chromium 메모리가 제한적 → 스냅샷 최소화가 더욱 중요
- Pi 4 2GB 이상 권장, Pi 3B+는 느림
- swap 설정 필수 (2GB 미만 RAM)

**Antigravity 추가 사항:**
- 인증: `openclaw models auth login --provider google-antigravity`
- Google Cloud 프로젝트 할당량 사용
